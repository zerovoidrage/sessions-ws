...
// ВАЖНО: PORT не должен быть равен RTMP_PORT (1935)
// Railway автоматически устанавливает PORT, но если он равен 1935, это неправильная конфигурация
const RTMP_PORT = parseInt(process.env.RTMP_PORT || '1935', 10)
const PORT = (() => {
  const port = Number(process.env.PORT || process.env.WS_PORT || 3001)
  if (port === RTMP_PORT) {
    console.error(`[WS-SERVER] ⚠️ PORT=${port} (RTMP port). HTTP/WebSocket server will listen on this port.`)
    console.error(`[WS-SERVER] ⚠️ RTMP server will not be able to start on port ${RTMP_PORT}.`)
    console.error(`[WS-SERVER] ⚠️ To fix: In Railway Settings → Networking, set HTTP/WebSocket port to DEFAULT, and add separate TCP proxy on port ${RTMP_PORT} for RTMP.`)
  }
  return port
})()
...
server.on('request', (req, res) => {
  ...
  // API endpoint для active speaker tracking
  if (req.url === '/api/active-speaker' && req.method === 'POST') {
    let body = ''
    req.on('data', (chunk) => { body += chunk.toString() })
    req.on('end', async () => {
      try {
        const data = JSON.parse(body)
        const { sessionSlug, identity, name } = data
        
        if (!sessionSlug || !identity) {
          res.statusCode = 400
          res.end(JSON.stringify({ error: 'Missing sessionSlug or identity' }))
          return
        }

        // Импортируем active speaker tracker
        const { updateActiveSpeaker } = await import('./active-speaker-tracker.js')
        await updateActiveSpeaker(sessionSlug, identity, true, name)
        
        res.statusCode = 200
        res.end(JSON.stringify({ success: true }))
      } catch (error: any) {
        console.error('[WS-SERVER] Error updating active speaker:', error)
        res.statusCode = 500
        res.end(JSON.stringify({ error: error.message || 'Failed to update active speaker' }))
      }
    })
    return
  }
  ...
  if (req.url === '/' && req.method === 'GET') {
    ...
    res.end(JSON.stringify({
      ...
      endpoints: {
        ...
        activeSpeaker: 'POST /api/active-speaker'
      },
      timestamp: new Date().toISOString()
    }))
    return
...
server.listen(PORT, async () => {
  ...
  // Проверяем, что RTMP порт не занят HTTP/WebSocket сервером
  if (PORT === RTMP_PORT) {
    console.error(`[WS-SERVER] ⚠️ Skipping RTMP server startup: PORT=${PORT} (HTTP/WebSocket server is already using this port)`)
